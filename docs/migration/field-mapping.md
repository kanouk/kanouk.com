...
全1,854コンテンツの変換監査は17,050ブロック、通常の`htmlBlock` 0件です。今後未知ブロックが見つかった場合も、即座にHTMLへ固定せず、意味ブロック追加か安全な標準表現への分解を優先します。
...
旧WordPress記事は可能な限りpathを維持します。旧domain側の301とcustom domain DNSは、全件監査後に別途ユーザーが切替を承認した時だけ設定します。
