...
export function startPerformanceAudit() {
 ...
 let longTasks=0;
 let organizerReadyMs: number | null = null;
 const captureReady=()=>{
  if(organizerReadyMs===null && document.querySelector('[data-organizer-ready="true"]')) organizerReadyMs=Math.round(performance.now());
 };
 captureReady();
 const readiness=new MutationObserver(captureReady);
 readiness.observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['data-organizer-ready']});
 ...
 setTimeout(()=>{clearInterval(timer);readiness.disconnect();},60000);
}
