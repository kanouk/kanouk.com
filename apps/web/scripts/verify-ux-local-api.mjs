...
const loaded = await payload(await api(`/_emdash/api/content/photos/${photo.id}?locale=ja`));
const change = (caption) => ({ method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ _rev: loaded._rev, data: { caption } }) });
await payload(await api(`/_emdash/api/content/photos/${photo.id}?locale=ja`, change('First editor saved this caption')));
assert.equal((await api(`/_emdash/api/content/photos/${photo.id}?locale=ja`, change('Stale editor must not overwrite'))).status, 409);
const readback = await payload(await api(`/_emdash/api/content/photos/${photo.id}?locale=ja`));
assert.equal(readback.item.data.caption, 'First editor saved this caption');
