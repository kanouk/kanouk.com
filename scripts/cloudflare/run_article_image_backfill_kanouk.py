#!/usr/bin/env python3
"""Run the bounded article-image backfill against pinned kanouk EmDash."""
...
BACKFILL_SCRIPT = WEB_ROOT / "scripts/wordpress/backfill-article-images.mjs"
...
def main() -> int:
    try:
        ...
        preflight(env)
        print(
            "EmDash article-image guard passed: admin token / pinned staging / "
        ...
        print(f"Article-image backfill guard blocked the command: {exc}", file=sys.stderr)
        return 2
