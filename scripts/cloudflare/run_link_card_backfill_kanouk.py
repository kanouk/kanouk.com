#!/usr/bin/env python3
"""Run the bounded link-card backfill against pinned kanouk EmDash."""
...
BACKFILL_SCRIPT = WEB_ROOT / "scripts/wordpress/backfill-link-cards.mjs"
...
def main() -> int:
    try:
        ...
        preflight(env)
        print(
            "EmDash link-card guard passed: admin token / pinned staging / "
        ...
        print(f"Link-card backfill guard blocked the command: {exc}", file=sys.stderr)
        return 2
