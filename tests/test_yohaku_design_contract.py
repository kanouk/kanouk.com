...
class YohakuDesignContractTests(unittest.TestCase):
    ...
    def test_album_photos_flow_left_to_right_in_chronological_order(self) -> None:
        ...
        self.assertIn("grid-template-columns: repeat(5, minmax(0, 1fr))", declarations)
        self.assertRegex(css, r"\.photo-grid\[data-custom-size\]\s*\{[^}]*grid-template-columns:\s*repeat\(auto-fill")
    ...
    def test_home_cards_and_sidebar_share_the_requested_visual_language(self) -> None:
        ...
        theme = (WEB_ROOT / "src/styles/theme.css").read_text()
        self.assertIn("getPostExcerpt", home)
        self.assertIn("excerpt={getPostExcerpt", home)
        self.assertIn('variant="list"', home)
        self.assertLess(card.index('class="card-media"'), card.index('class="card-body"'))
        self.assertIn('class="card-category"', card)
        self.assertNotIn('class="card-categories"', card)
        self.assertLess(card.index('class="card-meta"'), card.index('class="card-category"'))
